var username="";
let restraurant=[
    {id:"kendeji",chineseid:"肯德基",score:[0,0,0,0,0,0,0],average:0,location:"随处可见"},
    {id:"maidanglao",chineseid:"麦当劳",score:[0,0,0,0,0,0,0],average:0,location:"随处可见"},
    {id:"hanbaowang",chineseid:"汉堡王",score:[0,0,0,0,0,0,0],average:0,location:"随处可见"},
    {id:"saliya",chineseid:"萨莉亚",score:[0,0,0,0,0,0,0],average:0,location:"随处可见"}
]
...
function bodyOutput4(){
    var title1 = document.createElement("span");
    title1.className="title";
    title1.textContent = restraurant[3].chineseid; 
    document.getElementById("title4").appendChild(title1); 
    var s=0;
    var len=7;
    for(i=0;i<7;i++){
        if(restraurant[3].score[i]) s+=restraurant[3].score[i];  
        else len--;
    }
    s/=len;
    s = s.toFixed(1);
    localStorage.setItem("average"+restraurant[3].id, s);
    var score = document.createElement("span");
    score.textContent = "评分： " + s; 
    document.getElementById("score4").appendChild(score); 
    var l = document.createElement("span");
    l.textContent = "地理位置： " + restraurant[3].location; 
    document.getElementById("location4").appendChild(l); 
}
function reScore4(){
    var score=document.getElementById("score_field4").value;
	const dialog = document.getElementById('dialog-success');
    const cancelBtn = document.getElementById('cancelBtn');
    dialog.showModal();
    localStorage.setItem(username+restraurant[3].id, score);
    cancelBtn.addEventListener('click', () => {
      dialog.close();
    });
}